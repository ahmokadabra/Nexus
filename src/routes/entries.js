...
const entrySchema = z
  ...
  .refine(d => d.endMin > d.startMin, { message: 'endMin must be > startMin' });

function weekConflictFilter(newWeekType) {
  if (!newWeekType || newWeekType === 'ALL') return {};
...
function overlapCondition(startMin, endMin) {
  return {
    NOT: [{ endMin: { lte: startMin } }, { startMin: { gte: endMin } }],
  };
}
...
router.post('/', async (req, res) => {
  ...
  if (data.groupName) {
    groupConflicts = await prisma.scheduleEntry.findMany({
      where: { ...baseWhere, ...weekFilter, groupName: data.groupName },
      include: { course: { include: { subject: true } }, room: true, professor: true },
    });
  }

  if (roomConflicts.length + profConflicts.length + groupConflicts.length > 0) {
    return res.status(409).json({
      message: 'Konflikt u rasporedu',
      conflicts: { room: roomConflicts, professor: profConflicts, group: groupConflicts },
    });
  }
...
router.get('/:id', async (req, res) => {
  const item = await prisma.scheduleEntry.findUnique({
    where: { id: req.params.id },
    include: { course: { include: { subject: true } }, professor: true, room: true },
  });
