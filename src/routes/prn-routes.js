...
router.get("/plan", async (req, res) => {
  ...
  // pokušaj naći plan po kompozitnom ključu
  let plan = await prisma.pRNPlan.findUnique({
    where: { programId_yearNumber: { programId, yearNumber: year } },
  });

  // ako ne postoji, kreiraj i popuni redove prema SubjectProgram (godina)
  if (!plan) {
    ...
    if (uniqueSubjectIds.length) {
      await prisma.pRNRow.createMany({
        ...
        skipDuplicates: true, // potrebno imati @@unique([planId, subjectId]) u modelu
      });
    }
  }
  ...
  if (!full) return res.status(404).json({ message: "Plan not found after create" });
